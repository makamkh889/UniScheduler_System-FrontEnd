...
export class ApiStudentService {
  ...
  currentStudentData(): Observable<currentStudentData> {
    return this.httpclient.get<currentStudentData>(`${this.baseUrl}/currentStudentData`);

  }

  StuedntAcademicRecordCourses(): Observable<RegisterCourses[]> {
    return this.httpclient.get<RegisterCourses[]>(`${this.baseUrl}/StuedntAcademicRecordCourses`);
  }

  /////////////////Do not sure of this Observable<Schedule[]> 
  GetStudentSchedule(): Observable<Schedule[]> {
    return this.httpclient.get<Schedule[]>(`${this.baseUrl}/GetStudentSchedule`);

  }

  // take admin type and return Observable from type Admin
  StudentRecord(courseCode: string[]): Observable<any> {
    return this.httpclient.post<any>(`${this.baseUrl}/StudentRecord`, courseCode )
      .pipe(
        tap(response => {
